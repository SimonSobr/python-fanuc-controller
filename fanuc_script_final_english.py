...
APP_TITLE = "FANUC Collaborative Robot - Python Control"
APP_SUBTITLE = "Collaborative robot batch sorting with gesture and keyboard controls"
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        title.setObjectName("HeroTitle")
        title.setStyleSheet("font-size:28px;")
        subtitle = QLabel("Change camera, config path and robot mode")
        subtitle.setObjectName("HeroSubtitle")
        subtitle.setWordWrap(True)
        root.addWidget(title)
        root.addWidget(subtitle)
...
class MainWindow(QMainWindow):
    ...
    def _build_right_column(self):
        ...
        tray_card = CardFrame("Digital twin tray", accent="#38bdf8")
        ...
        self.route_cards = {}
        route_specs = [
            ("OK", "→", "OK", "Key 1 / swipe right"),
            ("REDO1", "↑", "REDO1", "Key 2 / swipe up"),
            ("REDO2", "↓", "REDO2", "Key 3 / swipe down"),
            ("NOK", "←", "NOK", "Key 4 / swipe left"),
        ]
    ...
    def _sync_ui(self):
        ...
        legend_lines.append(
            "<br><br><b>Tray logic:</b><br>"
            "Capacity is checked on confirm. If the current tray is full, the app offers either batch reset or tray replacement with queued execution."
        )
        if self.selected_mode == MODE_GESTURE:
            legend_lines.append(
                "<br><br><b>Gesture mode:</b><br>"
                "Hold canonical 1..5 to lock count, then swipe to choose category.<br>"
                "Confirm gesture = (False, False, True, True, True)<br>"
                "Undo gesture = (True, True, False, False, False)<br>"
                "New batch gesture = (True, False, False, False, True)"
            )
        else:
        ...
        if self.rt["state"] == "WAIT_COUNT":
            ...
            if self.selected_mode == MODE_GESTURE:
                ...
                hint = f"Locked {self.rt['locked_count']}. Press 1=OK, 2=REDO1, 3=REDO2, 4=NOK."
    ...
    def _handle_classic_category_choice(self, category_number: int):
        ...
        print(f"COUNT={self.rt['locked_count']}, CATEGORY_KEY={category_number}, CATEGORY={category}")
        self.apply_category_decision(self.rt["locked_count"], category)
    ...
    def _start_execution(self, plan, batch_counts, start_message: str = "Executing batch..."):
    ...
    def handle_confirm(self):
        ...
        if not fits:
            self.append_log("Confirm blocked: tray capacity exceeded. Waiting for operator decision.")
            dialog = TrayCapacityDialog(shortages, self.tray_counts, batch_counts, self)
            if dialog.exec() == QDialog.DialogCode.Accepted:
                self._reset_tray_counts()
                self.append_log("Tray replaced by operator. Digital twin tray state reset to empty.")
                self._start_execution(plan, batch_counts, "Tray replaced. Executing queued batch...")
            else:
                self.rt = reset_runtime()
                self.show_message("Batch reset. Current tray kept.")
                self.append_log("Operator canceled queued batch because tray had insufficient capacity.")
                print("Batch reset because tray capacity was exceeded.")
            return
        ...
        if self.selected_mode == MODE_CLASSIC and self.rt["state"] == "WAIT_COUNT":
            ...
            if key in digit_map:
                ...
                if 1 <= count <= remaining:
                    ...
                    self.rt["state"] = "WAIT_DIRECTION"
                    self.show_message(f"Count locked: {count}. Choose category with 1..4.")
                    print(f"LOCKED COUNT = {self.rt['locked_count']}")
                else:
